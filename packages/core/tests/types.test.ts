...
import type { ActionFn, ActionFnReturn, ActionInfo } from "../src/types/action";
import type {
  Actions as ActionsType,
  StoreActions,
} from "../src/types/actions";
import { AnyStoresStates } from "../src/types/any-store";
import {
  KeysOfActionStores,
  OnlyActionStores,
} from "../src/types/action-store";
...
describe("types", () => {
  ...
  it("tag utilities produce correct literals", () => {
    ...
    expectTypeOf<"wtest">().toExtend<WritableTag>();
    expectTypeOf<"rtest">().toExtend<ReadableTag>();
  });
  ...
  it("any-store utilities infer state types", () => {
    ...
    expectTypeOf<AnyStoresStates<[NStore, SStore]>>().toEqualTypeOf<
      [number, string]
    >();
  });
  ...
  it("action-store helpers extract action store states and keys", () => {
    ...
    expectTypeOf<OnlyActionStores<Stores>>().toEqualTypeOf<{
      a: number;
      r: never;
    }>();
    expectTypeOf<KeysOfActionStores<Stores>>().toEqualTypeOf<"a">();
  });
  ...
  it("adapter returns adapter store", () => {
    ...
    expectTypeOf<AdapterStore<number>>().toExtend<
  ...
  it("core utility types work", () => {
    ...
    expectTypeOf<
      OmitFirstArg<(a: number, b: string, c: boolean) => void>
    >().toEqualTypeOf<[b: string, c: boolean]>();
    expectTypeOf<
      KeysOfObject<{ a: number; b: never; c: string }>
    >().toEqualTypeOf<"a" | "c">();
  });
  ...
  it("action helpers enforce state types", () => {
    ...
    expectTypeOf<ActionFn<NStore>>().toEqualTypeOf<
      (state: Freeze<number>, ...v: any[]) => ActionFnReturn<number, "ws">
    >();
    expectTypeOf<ActionFnReturn<number, "ws">>().toEqualTypeOf<
  ...
  it("actions map functions without state argument", () => {
    ...
    expectTypeOf<ActionsType<S, Pre>>().toEqualTypeOf<{
      inc: (step: number) => void;
    }>();

    expectTypeOf<StoreActions<S>>().toExtend<{
      store: S;
      off(): void;
      set(newState: any, info?: ActionInfo): void;
    }>();
  });
});
